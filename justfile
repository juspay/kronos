...
smithy-build: smithy-validate
    #!/usr/bin/env bash
    # Recipe uses a shebang so the body runs as ONE shell process. This is
    # required because we issue compound `grep ... || cat <<EOF` blocks with
    # heredocs, which can't be split across just's per-line shell invocations.
    set -euo pipefail
    cd smithy && smithy build
    cd ..
    rm -rf crates/client
    cp -R smithy/build/smithy/source/rust-client-codegen crates/client
    # ── Restore hand-authored files the codegen wipe removed ────────────────
    # These files exist in git only — codegen never produces them, so they
    # come back via `git checkout`. If you add a new one, list it here too.
    git checkout -- crates/client/README.md
    git checkout -- crates/client/src/auth_ext.rs
    git checkout -- crates/client/tests/basic_auth_smoke.rs
    # ── Re-apply hand-authored edits to FILES THAT CODEGEN ALSO PRODUCES ────
    # `lib.rs` and `Cargo.toml` are regenerated on every run, so we cannot
    # `git checkout` them (that would discard legitimate codegen diffs — new
    # operations, new smithy deps, etc.). Instead we APPEND our additions
    # idempotently: each `grep -q ... || cat <<EOF >> ...` is a no-op when the
    # marker is already present, so future regens don't double-insert.
    #
    # If you ever change the appended content, change the grep marker too so
    # the next regen rewrites the section. Otherwise stale hand-edits linger.
    grep -q '^pub mod auth_ext;' crates/client/src/lib.rs || cat >> crates/client/src/lib.rs <<'EOF'

    // ── Hand-authored side module ──────────────────────────────────────────
    // NOT generated by codegen. The justfile's `smithy-build` recipe re-appends
