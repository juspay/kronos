...
> **IMPORTANT — openidconnect v4 API note:** The workspace pins
> `openidconnect = "4"` (post-scaffold dep bump). The example code below
> uses `openidconnect::reqwest::async_http_client` as the `discover_async`
> / `fetch_async` HTTP-client argument — that function was **removed in
> v4**. In v4 you construct a `reqwest::Client` and pass `&client` to
> these calls (the second argument now takes anything implementing
> `openidconnect::AsyncHttpClient`, and `&reqwest::Client` does).
>
> Before writing code, the implementer should briefly consult
> <https://docs.rs/openidconnect/4.0.1/openidconnect/> to confirm the
> exact `discover_async` / `fetch_async` signatures, then adapt the
> snippets below accordingly. Typical changes:
>
> 1. Replace `use openidconnect::reqwest::async_http_client;` with a
>    `reqwest::Client` field on `ValidatorInner` (built once in `new`).
> 2. Pass `&self.http` to `CoreProviderMetadata::discover_async` and
>    `CoreJsonWebKeySet::fetch_async`.
> 3. The same applies to the background refresh loop and `refresh_jwks`.
>
> Everything else (claim extraction, audience/issuer validation, scope
> normalisation, the `MockIdp` test harness) stays as written.
>
> Similarly, `jsonwebtoken = "10"` is mostly source-compatible with v9 at
> the surface we use (`decode_header`, `DecodingKey::from_rsa_components`,
> `Validation::new`, `decode::<T>`, `Algorithm::RS256` …). If anything
> doesn't compile, consult <https://docs.rs/jsonwebtoken/10.4.0/>.
