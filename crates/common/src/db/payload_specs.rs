...
pub async fn create(pool: &PgPool, name: &str, schema: &serde_json::Value) -> Result<PayloadSpec, sqlx::Error> {
...
pub async fn list(pool: &PgPool, cursor: Option<&str>, limit: i64) -> Result<Vec<PayloadSpec>, sqlx::Error> {
    match cursor {
        Some(c) => {
            sqlx::query_as::<_, PayloadSpec>(
...
pub async fn update(pool: &PgPool, name: &str, schema: &serde_json::Value) -> Result<Option<PayloadSpec>, sqlx::Error> {
    sqlx::query_as::<_, PayloadSpec>(
...
pub async fn has_dependent_endpoints(pool: &PgPool, name: &str) -> Result<bool, sqlx::Error> {
    let row: (i64,) = sqlx::query_as(
        "SELECT COUNT(*) FROM endpoints WHERE payload_spec_ref = $1"
    )
    .bind(name)
    .fetch_one(pool)
    .await?;
